...
| `sys_numlock_enables_numpad`                  |          | `0`           | obtained via active `LED_NUML` of keyboard device<br><br>enable with `1`, by default NumberPad reflects only disabling system numlock (then is disabled)                                                                                                   | `numpad_disables_sys_numlock`                  |          | `1`           | disabling system NumLock when is NumberPad disabled may be unwanted when is external keyboard connected
| **Key layout**                                |          |
