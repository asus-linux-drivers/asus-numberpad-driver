...
if [ -z "$LAYOUT_NAME" ]; then
    ...
    echo " - Standard. For all fields of NumberPad layout will try to found appropriate keys that are in success sent. Key modifiers are supported. Is required to have bound used special characters (%, *, #, ., -, +, /) with usual keysyms (percent, asterisk, numbersign, period, minus, plus, slash). When is appropriate key not found than field will be sent using the unicode shortcut (usually Ctrl+Shift+U+<0-F>)."
