...
if [ -z "$LAYOUT_NAME" ]; then
    ...
    echo " - Standard. All keys are sent directly except not found (modifiers are supported), which is why the unicode variant exists"
    echo " - The unicode variant sends not number characters (%, #, ., -, +, *, /) using the unicode shortcut (usually Ctrl+Shift+U+<0-F>). This layout is the most resistant to missing bindings of keys but sends multiple keys instead of just one, unnecessarily heavy if you do not need it."
    echo
