...
case "$RESPONSE" in [yY][eE][sS]|[yY])
    ...
    #SERVICE_INSTALL_DIR_PATH="/usr/lib/systemd/user" TODO:
    SERVICE_INSTALL_DIR2_PATH="/etc/systemd/user"
    ...
    if [ "$XDG_SESSION_TYPE" = "x11" ]; then
        cat "$SERVICE_X11_FILE_PATH" | USER=$USER LAYOUT_NAME=$LAYOUT_NAME CONFIG_FILE_DIR_PATH="$CONFIG_FILE_DIR_PATH/" DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR DBUS_SESSION_BUS_ADDRESS=$DBUS_SESSION_BUS_ADDRESS ERROR_LOG_FILE_PATH=$ERROR_LOG_FILE_PATH envsubst '$USER $LAYOUT_NAME $CONFIG_FILE_DIR_PATH $DISPLAY $XAUTHORITY $XDG_RUNTIME_DIR $DBUS_SESSION_BUS_ADDRESS $ERROR_LOG_FILE_PATH' | sudo tee "$SERVICE_INSTALL_DIR2_PATH/$SERVICE_INSTALL_FILE_NAME" >/dev/null
